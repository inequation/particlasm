...
outfile = open("libparticlasm.inc", 'w')
outfile.write("; particlasm NASM declarations\n")
outfile.write("; Copyright (C) 2011-2012, Leszek Godlewski <lg@inequation.org>\n\n")
...
# generate module prototypes, if requested
if (len(sys.argv) > 1 and sys.argv[1] == "-p"):
	print "Generating prototypes..."
	outfile = open("prototypes.tmp", 'w')
	for ei in enums:
		if (ei["symbol"] == "ptcModuleID"):
			...
			for p in ei["pairs"]:
				...
				outfile.write(".end:\n")
				outfile.write("\nmod_" + symbol + "Measure:\n")
				outfile.write("\tmov\tebx, 0\n")
				outfile.write("\tmov\tecx, 0\n")
				outfile.write("\tmov\tedx, 0\n")
				outfile.write("\tret\n")
				outfile.write("\nmod_" + symbol + "Compile:\n")
				outfile.write("\tret\n")
			outfile.write("\n")
		elif (ei["symbol"] == "ptcDistributionID"):
			outfile.write("; distributions\n")
			for prefix in ["s", "v", "c"]:
				for p in ei["pairs"]:
					symbol = p["name"][p["name"].find("_") + 1 : len(p["name"])]
					outfile.write("\n" + prefix + "distr_" + symbol + ":\n")
					outfile.write("\tnop\n")
					outfile.write(".end:\n")
					outfile.write("\tnop\n")
					outfile.write(prefix + "distr_" + symbol + "_size\tequ\t(" + prefix + "distr_" + symbol + ".end - " + prefix + "distr_" + symbol + " + 1)\n")
			outfile.write("\n")
	outfile.close()
	print "Done."
